...
- (void)setData:(NSArray *)data {
    ...
    if(! [_data isEqualToArray:data]) {
        ...
        self.scrollView.contentSize = CGSizeMake((data.count+2)*kScreenWidth,_scrollView.frame.size.height);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    NSInteger index = (NSInteger) (offsetX/kScreenWidth);
    if(index > _imageCount) {
        scrollView.contentOffset = CGPointMake(kScreenWidth,0);
    } else if(index == 0) {
        scrollView.contentOffset = CGPointMake(_imageCount * kScreenWidth,0);
    }
    if(index ==0) {
...
- (void) autoScroll {
    [UIView animateWithDuration:0.4 animations:^(){
        [self.scrollView setContentOffset:CGPointMake((self.pageControl.currentPage+2)*kScreenWidth,0)];
