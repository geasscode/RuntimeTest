...
- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles selectedBtn:(SelectedBlock)block{
    ...
    if (self = [super initWithFrame:frame]) {
		...
		newsTitle =  [NSMutableArray array];
...
- (void)CreateView:(CGRect)frame title:(NSArray *)hostlist{
    ...
    for (int i = 0; i<titleArray.count; i++) {
		...
		TitleModel * model = (TitleModel *)[titleArray objectAtIndex:i];
        ...
        //根据文字大小  计算按钮的宽度
        CGFloat length = [model.title boundingRectWithSize:CGSizeMake(320, 50) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size.width;
        btn.tag = i + 100;
        [btn addTarget:self action:@selector(itemSelected:) forControlEvents:UIControlEventTouchUpInside];
        btn.frame = CGRectMake(btnW, 0, length+20, 50);
		[newsTitle addObject:model.title];
...
- (void)changeItem:(UIButton *)sender {
    ...
    if (!_btncollectionView) {
        ...
        _btncollectionView.titles = newsTitle;
